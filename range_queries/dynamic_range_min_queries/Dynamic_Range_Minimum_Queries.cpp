...
class SegmentTree{
    private:
        ...
        void build(ll level, ll l, ll r){
            ...
            segmentTree[level] = min(segmentTree[2*level],segmentTree[2*level + 1]);
        }
    ...
    public:
        ...
        ll getTheMin(ll level, ll l, ll r, ll ql, ll qr){
            if(l > qr || r < ql) return LLONG_MAX;
            if(ql <= l && r <= qr) return segmentTree[level];

            ll m = l + (r - l) / 2;
            return min(getTheMin(2*level, l, m, ql, qr), getTheMin(2*level + 1, m+1, r, ql, qr));
        }
...
int main(){
    ...
    while(q -- ){
        ...
        if(tp == 2){
            ll a, b;
            cin >> a >> b;
            ll s = segmentTree.getTheMin(1, l, r, a-1, b-1);
            cout << s << endl;
        }
    }
}